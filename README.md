...
│
├── src/
│   ├── data_loader.py        # Handles preprocessing and data generators
│   ├── model.py              # Defines ResNet50-based architecture
...
Images are resized to 224 × 224 pixels and normalized to the range [0, 1]. Data augmentation techniques such as rotation, zoom, and horizontal flipping are applied to improve generalization. Validation and test datasets remain unmodified to ensure unbiased evaluation.

---

### Data Pipeline

Data is loaded using TensorFlow/Keras ImageDataGenerator, enabling efficient batch processing and automatic labeling based on directory structure. Separate generators are created for training, validation, and testing.

---

### Model Architecture

Transfer learning is implemented using ResNet50 pretrained on ImageNet.

- Convolutional base is loaded without the top layer  
- Base layers are frozen to preserve learned features  
